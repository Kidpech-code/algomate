...
final class NoStrategyFailure extends AlgoMateFailure {
  ...
  factory NoStrategyFailure.forSignature(String signature) =>
      NoStrategyFailure('No strategy found for signature: $signature', 'Consider registering built-in strategies or adjusting your selector hint.');
}
...
final class InapplicableInputFailure extends AlgoMateFailure {
  ...
  factory InapplicableInputFailure.requiresSorted(String strategyName) => InapplicableInputFailure(
  ...
  factory InapplicableInputFailure.sizeLimit(String strategyName, int limit) =>
      InapplicableInputFailure('Strategy "$strategyName" has size limit of $limit', 'Use a different strategy or reduce input size.');
}
...
final class ResourceLimitFailure extends AlgoMateFailure {
  ...
  factory ResourceLimitFailure.memoryBudget(int required, int budget) => ResourceLimitFailure(
  ...
  factory ResourceLimitFailure.timeout(Duration timeout) =>
      ResourceLimitFailure('Operation timed out after ${timeout.inMilliseconds}ms', 'Consider using async execution or increasing timeout.');
}
...
final class ExecutionFailure extends AlgoMateFailure {
  ...
  factory ExecutionFailure.strategyError(String strategyName, Object error) =>
      ExecutionFailure('Strategy "$strategyName" failed during execution', error.toString());

  factory ExecutionFailure.fallbackExhausted(List<String> attemptedStrategies) => ExecutionFailure(
...
final class IsolateFailure extends AlgoMateFailure {
  ...
  factory IsolateFailure.spawnError(Object error) => IsolateFailure('Failed to spawn isolate for async execution', error.toString());

  factory IsolateFailure.communicationError(Object error) => IsolateFailure('Communication error with isolate', error.toString());

  factory IsolateFailure.timeout(Duration timeout) =>
      IsolateFailure('Isolate execution timed out after ${timeout.inMilliseconds}ms', 'Consider increasing timeout or reducing workload size.');
}
...
final class BenchmarkFailure extends AlgoMateFailure {
  ...
  factory BenchmarkFailure.invalidConfiguration(String reason) => BenchmarkFailure('Invalid benchmark configuration', reason);

  factory BenchmarkFailure.executionError(String benchmarkName, Object error) =>
      BenchmarkFailure('Benchmark "$benchmarkName" execution failed', error.toString());

  factory BenchmarkFailure.insufficientData(int required, int actual) =>
      BenchmarkFailure('Insufficient benchmark iterations', 'Required at least $required iterations but only $actual completed successfully.');
}
...
final class MemoryLimitFailure extends AlgoMateFailure {
  ...
  factory MemoryLimitFailure.exceeded(int usedBytes, int limitBytes) => MemoryLimitFailure(
    'Memory limit exceeded',
    'Used ${(usedBytes / 1024 / 1024).toStringAsFixed(2)}MB but limit is ${(limitBytes / 1024 / 1024).toStringAsFixed(2)}MB',
  );

  factory MemoryLimitFailure.allocationFailed(int requestedBytes) => MemoryLimitFailure(
...
final class TimeoutFailure extends AlgoMateFailure {
  ...
  factory TimeoutFailure.operationTimeout(String operation, Duration timeout) => TimeoutFailure(
  ...
  factory TimeoutFailure.strategyTimeout(String strategyName, Duration timeout) => TimeoutFailure(
...
final class StrategyRegistrationFailure extends AlgoMateFailure {
  ...
  factory StrategyRegistrationFailure.duplicateName(String strategyName) => StrategyRegistrationFailure(
    'Strategy with name "$strategyName" already registered',
    'Use a different name or explicitly replace the existing strategy.',
  );

  factory StrategyRegistrationFailure.invalidMetadata(String reason) => StrategyRegistrationFailure('Invalid strategy metadata', reason);

  factory StrategyRegistrationFailure.incompatibleInterface(String expected, String actual) => StrategyRegistrationFailure(
